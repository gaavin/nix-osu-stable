...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    install -Dm644 osu-mime/osu-file-extensions.xml \
      "$out/share/mime/packages/osu-winello.xml"

    for size in 16 24 32 48 64 128 256; do
      install -Dm644 osu-wine.png \
        "$out/share/icons/hicolor/''${size}x''${size}/apps/osu-winello.png"
    done
    install -Dm644 osu-wine.png "$out/share/icons/osu-winello.png"
    runHook postInstall
  '';

  meta = {
    description = "MIME types and icon for osu!stable (winello-style)";
